...
function draw(svg, g, data, W, H, color, p) {
  ...
  if (p.showLabels) {
    ...
    // Le pas géométrique reste un plancher ; `labelDensity` éclaircit au-delà.
    const densiteEtq = dans01(p.labelDensity, 1);
    const pasDensiteEtq = densiteEtq > 0 ? Math.max(1, Math.round(1 / densiteEtq)) : Infinity;
    const every = Math.max(Math.max(1, Math.ceil((widest + 8) / stepX)), pasDensiteEtq);

    const etiquettes = g.selectAll('.stripe-label')
      .data(densiteEtq > 0 ? labels.filter((d, i) => i % every === 0) : [])
      .enter()
      .append('text')
