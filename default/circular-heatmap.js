...
function draw(svg, g, data, W, H, color, p) {
  ...
  // ---- Géométrie ---------------------------------------------------------
  // Masquable comme sur les deux autres visuels à échelle de couleur, et
  // escamotée d'office sous 170 px de haut où elle mangerait la couronne.
  const legendH = (p.showScale !== false && H > 170) ? 42 : 0;
  ...
  // tri des deux champs. Toutes ont la même emprise angulaire, donc pas de
  // notion de vitesse : elles apparaissent en fondu.
  const cellules = cg.selectAll('.cell')
    .data(data)
    .enter()
    .append('path')
    .attr('d', d => arc({
  ...
  if (anim) {
    cellules.attr('opacity', 0)
      ...
      .attr('opacity', d => opaciteDe(d.value));
  } else {
    cellules.attr('opacity', d => opaciteDe(d.value));
  }

  // ---- Valeurs dans les cellules ----------------------------------------
  // Éclaircissement des valeurs, comme sur la heatmap rectangulaire : la
  // densité porte sur une SURFACE, donc un secteur sur n ET un anneau sur n.
  const densiteEtq = dans01(p.labelDensity, 1);
  const pasEtq = densiteEtq > 0 ? Math.max(1, Math.round(1 / Math.sqrt(densiteEtq))) : Infinity;
  const rangTheta = new Map(sectors.map((v, i) => [v, i]));
  const rangR     = new Map(rings.map((v, i) => [v, i]));
  const etiquetees = densiteEtq > 0
    ? data.filter(d => rangTheta.get(d.theta) % pasEtq === 0 && rangR.get(d.r) % pasEtq === 0)
    : [];

  if (p.showLabels && etiquetees.length) {
    const etiquettes = cg.selectAll('.cell-label')
      .data(etiquetees)
      .enter()
      .append('text')
      .attr('transform', d => {
      ...
      .attr('fill', d => (d3.lab(couleurPercue(d.value)).l > 62 ? '#0f0f1a' : '#ffffff'))
  ...
  if (legendH) {
    ...
    // au-dessus d'une couronne estompée décrirait un rendu qui n'existe pas.
