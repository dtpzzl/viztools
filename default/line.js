...
function draw(svg, g, data, W, H, color, p) {
  ...
  // Ordonnée à l'origine : le bas de l'axe des valeurs. À 0 l'échelle part de
  // zéro et la longueur d'une marque représente la valeur. Réglée ailleurs,
  // elle tronque le bas et amplifie visuellement les écarts — précieux pour
  // lire de faibles variations, trompeur sur des barres. L'échelle est bornée :
  // une valeur sous l'origine se pose sur l'axe au lieu de sortir du cadre.
  const origine = Number.isFinite(+p.yOrigin) ? +p.yOrigin : 0;
  const yMax = Math.max(d3.max(data, d => d.value) * 1.15, origine + 1);
  ...
  if (p.showLabels ?? true) {
    // Éclaircissement des valeurs. Le pas GÉOMÉTRIQUE est un plancher : deux
    // étiquettes ne doivent jamais se toucher, même à densité maximale.
    // `labelDensity` éclaircit au-delà — 1 en pose autant que la place le
    // permet, 0,25 une sur quatre, 0 aucune.
    const dens01 = v => { const n = +v; return Number.isFinite(n) ? Math.max(0, Math.min(1, n)) : 1; };
    const densiteEtq = dens01(p.labelDensity);
    const pasLisibleEtq = Math.max(1, Math.ceil((d3.max(data, d => String(fmtY(d.value)).length) * ((p.fontSize ?? 12) - 1) * 0.62 + 10) / Math.max(1, (data.length > 1 ? W / (data.length - 1) : W))));
    const pasDensiteEtq = densiteEtq > 0 ? Math.max(1, Math.round(1 / densiteEtq)) : Infinity;
    const pasEtq = Math.max(pasLisibleEtq, pasDensiteEtq);
    const etiquetees = densiteEtq > 0 ? data.filter((d, i) => i % pasEtq === 0) : [];
