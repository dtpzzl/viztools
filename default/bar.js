...
function draw(svg, g, data, W, H, color, p) {
  ...
  const yMax = Math.max(d3.max(data, d => d.value) * 1.1, origine + 1);
  ...
  if (p.showLabels ?? true) {
    ...
    const pasLisibleEtq = Math.max(1, Math.ceil((d3.max(data, d => String(fmtY(d.value)).length) * ((p.fontSize ?? 12) - 1) * 0.62 + 10) / Math.max(1, x.step())));
